...
import { filter, timeout, throttleTime } from "rxjs/operators";
...
function parseArgs(): Uint8Array {
  ...
  // Relaxed for this probe: accept a 32-byte raw seed OR a 64-byte BIP-39 mnemonicToSeedSync
  // output (128 hex chars) — HDWallet.fromSeed() takes a plain Uint8Array with no documented
  // length constraint, and 64 bytes is the BIP-39 standard master-seed length.
  if (!/^[0-9a-fA-F]{64}$/.test(hex) && !/^[0-9a-fA-F]{128}$/.test(hex)) {
    console.error(
      "Error: seed must be 64 hex characters (32 bytes) or 128 hex characters (64 bytes)."
    );
    process.exit(1);
  }
...
async function main() {
  ...
  // ─── Step 3: Configuration (preprod probe) ─────────────────────────────────
  ...
  // Verbose diagnostic: log granular SyncProgress fields (not just isStrictlyComplete) every 5s,
  // for all three domains, to see whether dust is actually connected/advancing or truly stalled.
  const sub = wallet
    .state()
    .pipe(throttleTime(5_000))
    .subscribe((s) => {
      const fmt = (p: { appliedIndex: bigint; highestIndex: bigint; highestRelevantIndex: bigint; isConnected: boolean }) =>
        `connected=${p.isConnected} applied=${p.appliedIndex} highest=${p.highestIndex} highestRelevant=${p.highestRelevantIndex}`;
      console.log(
        `[${new Date().toISOString()}] shielded{${fmt(s.shielded.state.progress)}} ` +
          `unshielded{${fmt(s.unshielded.progress)}} dust{${fmt(s.dust.progress)}} dustBalance=${s.dust.balance(new Date())}`,
      );
    });
